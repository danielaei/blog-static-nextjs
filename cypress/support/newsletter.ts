Cypress.Commands.add("newsletter", () => {
    cy.get("[data-cy=newsletter]").within(function(){
        ...
        cy.get("[data-cy='error']").should("have.text","Please provide a valid email, Value is too short").and("have.class","text-red-600 peer")
        
        cy.get('input[placeholder*="Enter your email"]')
        
        cy.get("@inputNews").type("z@g")
        cy.get("@btnSubmit").click()
        cy.get("[data-cy='error']").should("have.text","Please provide a valid email").and("have.class","text-red-600 peer")
        cy.get("@inputNews").clear()
  
        cy.get("@inputNews").type("daniel.aghababaei@gmail.com")
        cy.get("@btnSubmit").click()
        cy.get("[data-cy='success']").should("have.text","Thank you for subscribing to our newsletter! 🎉").and("have.class","text-green-600 peer")
  
        cy.get("p").last().should("have.text","We care about your data in our privacy policy")
        cy.get("a").invoke("attr",'href').then(function($link){
