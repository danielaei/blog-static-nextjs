...
const ErrorMessages = ({ errors }: { errors: string[] }) => {
  ...
  return <span data-cy="error" className="text-red-600 peer">{text}</span>;
};
...
const SuccessMessages = ({ message }: { message: string|undefined }) => {
  ...
  return <span data-cy="success" className="text-green-600 peer">{message}</span>;
};
export const Newsletter = () => {
  const [state, formAction] = useFormState(newsletterAction, { errors: [] ,msg:""});
  const emailErrors = findErrors("email", state.errors);
  return (
    <div data-cy="newsletter" className="text-center space-y-5">
      ...
      >
        <input
          className="focus:outline-none rounded-lg py-2 px-4 md:w-1/4 mobileS:w-2/3 text-black"
          type="email"
          name="email"
          id="email"
